...
RSpec.describe 'PATCH /api/items/{id}' do
  ...
  let(:work){ create :work, category: 'magazine', creator: creator, language: languages[0], start_year: 2000, end_year: 2003 }
  let(:other_work){ create :work, category: 'magazine', creator: creator, language: languages[0], start_year: 2000, end_year: 2002 }
  let(:item){ create :issue, work: work, creator: creator, language: languages[0] }

  let! :original_version do
    IssuePolicy.new(:app, item).serializer.serialize
  end
  ...
  describe "with a work that has a start and end year" do
    let(:work){ create :work, category: 'magazine', creator: creator, language: languages[0], start_year: 2000, end_year: 2002 }
