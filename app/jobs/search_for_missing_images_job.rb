class SearchForMissingImagesJob
  ...
  def self.perform options = {}
    ...
    items = Item.where('image_id IS NULL AND main_image_search_id IS NULL').limit(n).to_a
    ...
    if items.length < n
      parts = ItemPart.where('image_id IS NULL AND main_image_search_id IS NULL').limit(n - items.length).to_a
